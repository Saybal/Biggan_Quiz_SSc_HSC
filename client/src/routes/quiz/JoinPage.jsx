...
export default function JoinPage() {
  ...
  const [participatedOnTime, setparticipatedOnTime] = useState(false);
  ...
  const qCount = examId
    ...
    : questions.filter(
        (q) => q.subjectId === selSubjectId && q.levelId === selLevelId,
      ).length;
  ...
  React.useEffect(() => {
    ...
    async function loadExam() {
      ...
      try {
        ...
        console.log(eRes.data.publishDate);
        setPublish(eRes.data.publishDate)
        ...
        if (user) {
          try {
            ...
            if (st.data?.attempt?.participatedOnTime)
              setparticipatedOnTime(true);
          } catch {
  ...
  React.useEffect(() => {
    ...
    // const publish = new Date(exam.publishDate);

    if (examId && alreadyDone && participatedOnTime) {
      Swal.fire({
        text: "তুমি ইতিমধ্যে এই পরীক্ষায় অংশগ্রহণ করেছো। তাই এখন শুধুমাত্র অনুশীলনের জন্য তুমি পরীক্ষা দিতে পারবে। এক্ষেত্রে তোমার মেরিট যুক্ত হবে না।",
        theme: "dark",
      });
    } else if (today > publish) {
      console.log("deadline crossed!!");
      
      Swal.fire({
        text: "তুমি এই পরীক্ষায় যথাসময়ে অংশগ্রহণ করতে পারোনি। তাই এখন শুধুমাত্র অনুশীলনের জন্য তুমি পরীক্ষা দিতে পারবে। এক্ষেত্রে তোমার মেরিট যুক্ত হবে না।",
        theme: "dark",
      });
    }
  }, [ exam]);
  ...
  const handleStart = () => {
    ...
    // }
    if (!name.trim()) {
      setError("⚠️ নাম লিখতে ভুলে গেছো!");
      return;
    }
    if (!school.trim()) {
      setError("⚠️ স্কুল/কলেজের নাম লিখো!");
      return;
    }
    ...
    if (quizOptions.randomQ) qs = [...qs].sort(() => Math.random() - 0.5);
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen px-4 py-8 z-10 screen-animate">
      <SelBg
        patternColor="rgba(67,233,123,0.05)"
        symbols={["⏱️", "🚀", "⚡", "🎯", "Δt", "v₀", "★", "☄"]}
      />
      <div className="relative z-10 flex flex-col items-center w-full max-w-[480px] px-4">
        ...
        <div className="text-center mb-5">
          ...
          <h2 className="font-display font-extrabold text-[2rem] text-textprimary mb-1">
            প্রস্তুত?
          </h2>
          <p className="text-muted text-[.9rem]">
            {exam
              ? `📌 ${exam.examName}`
              : `${subject?.emoji} ${subject?.name} · ${level?.name}`}{" "}
            · {qCount} প্রশ্ন
          </p>
        </div>
        ...
        <div className="text-center mb-5">
          <div className="relative w-[130px] h-[130px] mx-auto mb-2">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="font-display font-extrabold text-[1.6rem] gradient-text-green leading-none">
                {String(mins).padStart(2, "0")}:০০
              </span>
              <span className="text-muted text-[.62rem] mt-0.5 tracking-widest uppercase">
                মিনিট
              </span>
            </div>
          </div>
        ...
        <div
          className="w-full rounded-[18px] p-5 mb-2 relative overflow-hidden"
          style={{
            background: "var(--card)",
        ...
        >
          <div
            ...
            style={{
              background: "linear-gradient(90deg,#43e97b,#38b2f5,#a78bfa)",
            }}
          />
          ...
          <div className="mb-3">
            ...
            <input
              className="w-full bg-card2 border-[1.5px] border-border rounded-xl px-3.5 py-2.5 text-textprimary font-body text-[.95rem] outline-none transition-all focus:border-green placeholder-muted"
              type="text"
              placeholder="পুরো নাম লিখো..."
              maxLength={40}
              value={name}
              onChange={(e) => setName(e.target.value)}
              onKeyDown={(e) =>
                e.key === "Enter" &&
                document.getElementById("school-inp").focus()
              }
            />
          </div>
          <div className="mb-4">
          ...
          {/* || (examId && alreadyDone) eta disable e bosbe */}
          <button
            onClick={handleStart}
            disabled={
              examLoading || (examId ? examQuestions.length === 0 : false)
            }
            ...
            style={{
              background: "linear-gradient(135deg,#0e1f15,#0a1a28)",
              border: "1.5px solid rgba(67,233,123,.5)",
              boxShadow: "0 4px 22px rgba(67,233,123,.14)",
            }}
          >
            <span
              className="text-[2rem] flex-shrink-0"
              style={{ filter: "drop-shadow(0 0 8px rgba(67,233,123,.5))" }}
            >
              🚀
            </span>
            <span className="flex-1">
              <span className="block font-display font-extrabold text-[1.1rem] text-green">
                শুরু করো
              </span>
          ...
          {error && (
            <p className="text-accent2 text-xs mt-3 text-center">{error}</p>
          )}
          {examErr && (
            <p className="text-accent2 text-xs mt-3 text-center">{examErr}</p>
          )}
          {/* {alreadyDone && !examErr && (
        ...
        <Link
          to="/quiz/select-level"
          className="w-full py-2.5 bg-transparent border border-border text-muted font-display font-semibold rounded-xl text-sm text-center block transition hover:border-accent hover:text-accent"
        >
